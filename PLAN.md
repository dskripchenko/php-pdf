...
| **2. Bundled-шрифты** | **Liberation трио (Sans + Serif + Mono) + caller-provided API** | ~3 MB bundle; метрики совпадают с Arial/Times/Courier (важно для совместимости с DOCX-input'ом); caller может через FontProvider добавить любой TTF/OTF |
...
| Inline-runs: bold / italic / underline / strikethrough / sup / sub / color / size / fontFamily | Liberation Sans/Serif/Mono bundled + FontProvider API для caller-provided TTF/OTF |
...
- Bundled шрифты: Liberation Sans + Serif + Mono (Liberation License)
- FontProvider API для caller-provided TTF/OTF
