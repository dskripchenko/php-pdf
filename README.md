...
| HTML/CSS input                   | ⚠️ через AST mapper | ✅ | ⚠️ basic | ✅ | ❌ |
...
- Меньшая battle-tested база — mpdf existing 20+ лет, тысячи production
  deployments
- HTML/CSS input partial — нужно собрать AST вручную или через CSS mapper.
  Для прямого HTML→PDF mpdf удобнее out of the box
