...
- [x] **P10 — `PdfMerger::stamp()` (page-as-XObject embed).** Page-as-XObject with BBox from CropBox + `/Matrix` from Rotate, resource closure imported, placed via `Placement` (fit/stretch/at). — 5 tests. Implemented in the merge pipeline (own Form XObject value). The core-Writer FPDI path — combining imports with *freshly generated* php-pdf content — is now done too via `Merge\PageImporter::intoDocument()` + `Document::registerImportedForm()` + `Reader\PdfValueSerializer` (foreign objects injected into the writer with references renumbered).
- [x] **P11 — Docs + CHANGELOG.** User-facing `docs/{en,ru}/MERGE.md`, README Documentation link, CHANGELOG [Unreleased] entry. (zh/de translations: follow-up.)
