...
5. **AST/Converter strategy:** Self-contained `Html\Converter` + Element/
   Style namespace в php-pdf без внешних cross-package dependencies.
...
- Self-contained AST увеличивает code-volume, но изолирует
  cross-package coupling и упрощает versioning
...
4. **ADR-PDF-002** — Architecture Decision Record для production
   Document AST API (DocumentBuilder fluent API).
